...
const looks = function (isInitialSetup, isStage, targetId, costumeName, backdropName) {
    ...
    <category name="%{BKY_CATEGORY_LOOKS}" id="looks" colour="#9966FF" secondaryColour="#774DCB">
        ...
        ${isStage ? `
            ...
            <block type="looks_getbackdroplength"/>
        ` : `
            <block id="${targetId}_costumenumbername" type="looks_costumenumbername"/>
            <block id="backdropnumbername" type="looks_backdropnumbername"/>
            <block id="${targetId}_size" type="looks_size"/>
            <block id="${targetId}_getcostumelength" type="looks_getcostumelength"/>
            <block type="looks_getbackdroplength"/>
        `}
        ${categorySeparator}
    </category>
...
const liveTests = function (isLiveTest, targetId) {
    return `
...
const makeToolboxXML = function (isInitialSetup, isStage = true, targetId, categoriesXML = [],
    ...
    const menusXML = moveCategory('selectors') || selectors(isInitialSetup, isStage, targetId);
    const variablesXML = moveCategory('data') || variables(isInitialSetup, isStage, targetId);
    const listsXML = moveCategory('lists') || lists(isInitialSetup, isStage, targetId);
    const myBlocksXML = moveCategory('procedures') || myBlocks(isInitialSetup, isStage, targetId);
    const liveTestsXML = moveCategory('liveTests') || liveTests(isLiveTest, targetId);
