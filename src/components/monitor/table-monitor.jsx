...
const TableMonitor = ({draggable, label, width, height, value, onResizeMouseDown, ...rowProps}) => (
    <div
    ...
    >
        ...
        <div className={styles.listFooter}>
            <div className={classNames(styles.footerHidden, 'no-drag')}>
                {'?' /* really try and find a better way to do this */}
            </div>
            <div className={styles.footerLength}>
                <FormattedMessage
                    defaultMessage="items {item}"
                    description="Items label on table monitors. DO NOT translate {item} (with brackets)."
                    id="gui.monitor.tableMonitor.tableItem"
                    values={{
                        item: value.reduce((acc, row) => acc + row.length, 0)
                    }}
                />
            </div>
            <div
