export default async function ({ addon, console, msg, safeMsg }) {
  ...
  const separateVariablesByType = (toolboxXML) => {
    ...
    const tableButtonIndex = toolboxXML.findIndex(
      (i) => i.getAttribute("callbackkey") === "CREATE_TABLE" || i.getAttribute("type") === "data_addtotable"
    );
  ...
  const moveReportersDown = (toolboxXML) => {
    ...
    return moveReportersToEnd(variables)
      .concat(moveReportersToEnd(lists))
      .concat(moveReportersToEnd(tables));
  };
  ...
  const variableCategoryCallback = (workspace) => {
    ...
    const { variables, lists, tables } = separateVariablesByType(result);
  ...
  vm.runtime.getBlocksXML = function (target) {
    ...
    if (!addon.self.disabled && hasSeparateListAndTableCategories) {
      result.push({
        ...
        <category
          name="${safeMsg("table-category")}"
          id="tables"
          colour="${ScratchBlocks.Colours.data_tables.primary}"
          secondaryColour="${ScratchBlocks.Colours.data_tables.tertiary}"
          custom="TABLE">
        </category>`,
      });
      result.map = (callback) => {
