...
export default async function ({ addon, console, msg }) {
  ...
  addon.tab.addBlock("\u200B\u200Bcustom log\u200B\u200B %s // background color: %m", {
    args: ["content", "color1"],
    displayName: "custom log %s // background color: %m",
  ...
  addon.tab.addBlock("\u200B\u200Bstart timer\u200B\u200B %s", {
    ...
    callback: ({ content }, thread) => {
      if (timingTab) timingTab.startTimer(content, thread.target.id, thread.peekStack ? thread.peekStack() : thread.thread.peekStack());
    },
  });
